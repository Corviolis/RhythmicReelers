...
func setup_direction(window_size: Vector2):
	...
	if window_size.x > window_size.y: # horizontal
		...
		progress_bar.size = Vector2(window_size.x - 2, 1)
		progress_bar.fill_mode = ProgressBar.FILL_BEGIN_TO_END
	else: # vertical
		region_rect = Rect2(0, 0, 5, 3)
		patch_margin_left = 3
		progress_bar.position = Vector2(1, 1)
		progress_bar.size = Vector2(1, window_size.y - 2)
		progress_bar.fill_mode = ProgressBar.FILL_BOTTOM_TO_TOP
	progress_bar.value = 15

func place_window(window_position: Vector2, window_size: Vector2, _minigame_scene: PackedScene):
	size = Vector2(window_size.x, window_size.y)
	position = Vector2(window_position.x - int(window_size.x / 2), window_position.y - int(window_size.y / 2))
